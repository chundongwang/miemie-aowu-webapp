...
export default function PhotoSearchModal({ itemId, initialQuery, onPhotoAdded, onSelectUrl, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-white w-full max-w-md rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[85vh]"
      ...
      >
        ...
        <div className="px-4 pt-5 pb-3 shrink-0">
          ...
          <form onSubmit={handleSearch} className="flex gap-2">
            <input
              ...
              className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#2B4B8C]"
            />
            <button
              type="submit"
        ...
        <div className="overflow-y-auto px-4 pb-6 flex-1">
          {searching && (
            <p className="text-sm text-gray-400 text-center py-8">{t("searchPhotoSearching")}</p>
          )}

          {!searching && searched && results.length === 0 && (
            <p className="text-sm text-gray-400 text-center py-8">{t("searchPhotoNoResults")}</p>
          )}
              ...
              {results.map((r) => {
                ...
                return (
                  <button
                    ...
                    disabled={!!addingUrl}
                    className="relative aspect-square rounded-xl overflow-hidden bg-gray-100 disabled:opacity-50 hover:ring-2 hover:ring-[#2B4B8C] transition-all"
